...
def check_live_fire():
    ...
    try:
        ...
        for pattern in ("fable-code-*", "fable-playbook-*"):
            for s in glob.glob(os.path.join(tempfile.gettempdir(), pattern)):
                if "doctor-" in s:
                    try:
                        os.remove(s)
                    except OSError:
